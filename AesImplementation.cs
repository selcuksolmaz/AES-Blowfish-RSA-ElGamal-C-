...
namespace Encryption_Algorithms
{
    class AesImplementation
    {
        ...
        private static int _nb; // block size in 32-bit words.  Always 4 for AES.  (128 bits).
        private static int _nk; // key size in 32-bit words.  4, 6, 8.  (128, 192, 256 bits).
        private static int _nr; // number of rounds. 10, 12, 14.
        ...
        private static readonly int[] InverseSbox =
      {
...
/*f*/        0x60, 0x51, 0x7F, 0xA9, 0x19, 0xB5, 0x4A, 0x0D,0x2D, 0xE5, 0x7A, 0x9F, 0x93, 0xC9, 0x9C, 0xEF,
/*e*/        0xA0, 0xE0, 0x3B, 0x4D, 0xAE, 0x2A, 0xF5, 0xB0, 0xC8, 0xEB, 0xBB, 0x3C, 0x83, 0x53, 0x99, 0x61,
/*f*/        0x17, 0x2B, 0x04, 0x7E, 0xBA, 0x77, 0xD6, 0x26, 0xE1, 0x69, 0x14, 0x63, 0x55, 0x21, 0x0C, 0x7D
        };

        // matrix used to switch bytes during key expansion (0-255 values)
        ...
        private static byte[] Xor(byte[] a, byte[] b)//Xor Function
        {
        ...
        private static byte[][] GenerateSubkeys(byte[] key)
        {
            ...
            int i = 0;
            // Create the key for the first round
            while (i < _nk)
            {
                tmp[i][0] = key[i * 4];
                tmp[i][1] = key[i * 4 + 1];
            ...
            // Creating keys for other rounds, where every 4th column 
            // rotates on its own and replaces bytes in Rcon
            while (i < _nb * (_nr + 1))
            {
                ...
                if (i % _nk == 0)
                {
                    ...
                    temp[0] = (byte)(temp[0] ^ (Rcon[i / _nk] & 0xff));
                }
        ...
        private static byte[] SubWord(byte[] inBytes)
        {
            byte[] tmp = new byte[inBytes.Length];

            for (int i = 0; i < tmp.Length; i++)
                tmp[i] = (byte)(Sbox[inBytes[i] & 0xff] & 0xff);// Used "And" operator(bits) 0xff=255

            return tmp;
        }


        private static byte[] RotateWord(byte[] input) //Word is rotated
        {
            byte[] tmp = new byte[input.Length];
            tmp[0] = input[1];//substitution -1
            tmp[1] = input[2];
            tmp[2] = input[3];
            tmp[3] = input[0];
            return tmp;
        ...
        private static byte[][] AddRoundKey(byte[][] state, byte[][] w, int round)
        {
            ...
            {
                for (int l = 0; l < 4; l++)
                    tmp[l][c] = (byte)(state[l][c] ^ w[round * _nb + c][l]);
            }
        ...
        // Method for substituting state bytes
        ...
        private static byte[][] InvSubBytes(byte[][] state)
        {
            for (int row = 0; row < 4; row++)
                for (int col = 0; col < _nb; col++)
                    state[row][col] = (byte)(InverseSbox[(state[row][col] & 0xff)] & 0xff);

            return state;
        }

        private static byte[][] ShiftRows(byte[][] state)//For encrpt
        {
            byte[] t = new byte[4];
            for (int r = 1; r < 4; r++)
            {
        ...
        private static byte[][] InvShiftRows(byte[][] state)//For Decrypt
        {
            byte[] t = new byte[4];
            for (int r = 1; r < 4; r++)
            {
        ...
        private static byte[][] MixColumns(byte[][] s)//For Encrpt
        {
            int[] sp = new int[4];
        ...
        private static byte[][] InvMixColumns(byte[][] s)//For Decrypt
        {
            int[] sp = new int[4];
        ...
        public static byte FFMul(byte a, byte b)//Multiple
        {
        ...
        public static byte[] EncryptBlock(byte[] inBytes)
        {
            byte[] tmp = new byte[inBytes.Length];
            ...
            for (int i = 0; i < 16; i++)
                state[i / 4][i % 4] = inBytes[i % 4 * 4 + i / 4];

            state = AddRoundKey(state, _w, 0);
            for (int round = 1; round < _nr; round++)//Loop for Round value
            {
                state = SubBytes(state);
                state = ShiftRows(state);
                state = MixColumns(state);
                state = AddRoundKey(state, _w, round);
            }

            state = SubBytes(state);
            state = ShiftRows(state);
            state = AddRoundKey(state, _w, _nr);
        ...
        public static byte[] DecryptBlock(byte[] inBytes)
        {
            byte[] tmp = new byte[inBytes.Length];
        ...
        {
            int count = 0;
            //Count how many 0 you added to make a block cipher. Remove them
